# Installation guide — Conveyer Grok
...
- A **69labs.vip account** (paid subscription — covers Grok video generation)
- A **HeyGen account** (paid plan — for the TTS voiceover)
...
Node.js runs the Conveyer Grok code.
...
10–30 seconds. You should see progress lines ending with `Receiving objects: 100% ...`.
...
4. **Add an exclusion** → **Folder** → pick your `Conveyer-Grok` folder
...
## Part 3 — Get your API keys (3 keys total)
...
### Key 2 — 69labs (for Grok video generation)
...
### Key 3 — HeyGen (for voiceover TTS)

1. Open https://app.heygen.com/settings/api
2. Sign up / sign in to a plan that has API access
3. Copy your API key

Also grab a **voice_id** from HeyGen → Voices → click into a voice (use ElevenLabs or Panda engine, NOT Starfish — Starfish voices don't work for standalone TTS) → copy the voice_id (UUID-like string).
...
In your terminal (still inside the `Conveyer-Grok` folder):
...
You'll see the **Conveyer Grok** interface with a left sidebar.
...
2. Under **Required API Keys**, fill in:
   ...
   - `HEYGEN_API_KEY` — HeyGen key from Part 3 Key 3
   - `HEYGEN_VOICE_ID` — voice_id from HeyGen Voices
...
2. Sign in with your Google account
3. Project selector (top, next to the logo) → **New project** → name it "Conveyer Grok" → **Create**
...
2. **External** → **Create**
3. **App name** — "Conveyer Grok", **User support email** — your email
...
### Step 5 — Paste into Conveyer Grok
...
2. Go into the project folder:
   - Windows: `cd %USERPROFILE%\Documents\Conveyer-Grok`
   - macOS: `cd ~/Documents/Conveyer-Grok`
3. Pull latest code:
   ```
   git pull
   ```
...
**Your API keys, prompts, presets, and run history are NOT affected** — they live in `~/.conveyer-grok/` outside the project tree, so updates can never delete them.
...
Inside that folder:
- `grok.db` — SQLite database (settings, API keys, prompts, presets, run history)
...
> **macOS users:** the folder starts with a dot so Finder hides it. To see it: press **⌘ + Shift + .** (period) in Finder, or **⌘ + Shift + G** and paste `~/.conveyer-grok/`.
